...
def maybe_to_torch(d):
    # Converts the incoming data to a pytorch tensor
...
class SegmentationNetwork(NeuralNetwork):
    def __init__(self):
        ...
        # If there are 5 pooling layers, then the size of the patch must be divisible by 2 to the fifth power. For example, in a 2D network, 5 pooling on the X-axis and 6 pooling on the Y-axis, the patch size
        # It has to be divisible by 2 to the fifth power
        self.input_shape_must_be_divisible_by = None
        # in y this would be (32, 64)

        # That tells you whether it's a 2D convolution or a 3D convolution
        self.conv_op = None  # nn.Conv2d or nn.Conv3d

        # This tells us how many channels there are, which in reasoning is important for pre-allocating memory
    ...
    def predict_3D(self, x: np.ndarray, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
        ...
        # Previously, the image axis of the 3D network was (2, 3, 4), which was only used to block code that used this convention
        if len(mirror_axes):
    ...
    def predict_2D(self, x, do_mirroring: bool, mirror_axes: tuple = (0, 1, 2), use_sliding_window: bool = False,
        ...
        # Previously, the mirror axis of a 2D network was (2, 3), which was only used to block code that used this convention
        if len(mirror_axes):
            if max(mirror_axes) > 1:
                raise ValueError("mirror axes. duh")
    ...
    def _compute_steps_for_sliding_window(patch_size: Tuple[int, ...], image_size: Tuple[int, ...], step_size: float) -> List[List[int]]:
        ...
        # The maximum step size is patch_size*step_size, but it can be narrower. For example, the image size is 110, the patch size is 64, and the step size is 0.5. Then you want to run 3 steps starting from coordinates 0,23,46
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        # For sliding window inference, the image size must be at least the same as the patch size, and as long as the patch size is the same, it does not matter whether the shape of the image is divisible by the num_pool of 2
        data, slicer = pad_nd_image(
            x, patch_size, pad_border_mode, pad_kwargs, True, None)
        data_shape = data.shape  # still c, x, y, z

        # Calculate the step size of the sliding window
        ...
        # It only needs to be calculated once, and it may take some time to compute this due to the large sigma values in the Gaussian filter
        ...
        if all_in_gpu:
            # If you run inference only on the GPU, there is less CPU-GPU communication, but more GPU memory is required and some resources need to be pre-allocated on the GPU

            if use_gaussian and num_tiles > 1:
                # Half precision is enough for the output, and if the output here is half precision, the Gaussian importance diagram is also half precision.
                gaussian_importance_map = gaussian_importance_map.half()

                # Make sure you don't round to 0
        ...
        # Reverse the fill here (remember that the input is filled so that it is the same size as the patch)
        slicer = tuple(
        ...
        # Calculate the class probability by dividing the aggregate result by the number of samples (class_probabilities)
    ...
    def _internal_predict_2D_2Dconv(self, x: np.ndarray, min_size: Tuple[int, int], do_mirroring: bool,
        ...
        """
        This is a full convolution inference, not a sliding window
        """
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        # All operations here are done on the GPU, and if x and mult are not already on the GPU, they will be processed here, now returning a cuda tensor instead of a numpy array
    ...
    def _internal_predict_2D_2Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        # Backfill here (remember that the input is filled so that it is at least as large as the patch)
        slicer = tuple(
            [slice(0, aggregated_results.shape[i]) for i in
             range(len(aggregated_results.shape) - (len(slicer) - 1))] + slicer[1:])
        aggregated_results = aggregated_results[slicer]
        ...
        # Calculate the class probability by dividing the aggregate result by the number of result samples (class_probabilities)
        class_probabilities = aggregated_results / aggregated_nb_of_predictions

...
from ..networks.ActiveLoss import SemiActiveLoss


class ModelLossSemsegGatedCRF(torch.nn.Module):
    """
    This module provides an implementation of Gated CRF loss function for weakly supervised semantic segmentation. The loss function facilitates consistent label assignment through input features
...
class RobustCrossEntropyLoss(nn.CrossEntropyLoss):
    """
    This is a compatibility layer because the target tensor is floating-point and has an extra dimension
    """

    def forward(self, input: Tensor, target: Tensor) -> Tensor:
        input = input['pred_masks']
...
class TopKLoss(RobustCrossEntropyLoss):
    """
      Networks cannot have linear transformations!
    """
...
class MultipleOutputLoss2(nn.Module):
    def __init__(self, loss, weight_factors=None):
        """
        You can use this if you have multiple outputs and corresponding true values (both are lists of the same length) and you want to calculate the loss between them (x[0] and y[0], x[1] and y[1] etc)
        :param loss:
...
class GDL(nn.Module):
    def __init__(self, apply_nonlin=None, batch_dice=False, do_bg=True, smooth=1.,
        ...
        """
        The square_volumes parameter squares the weights. square_volumes=True; But the code author doesn't recommend it
        """
...
def get_tp_fp_fn_tn(net_output, gt, axes=None, mask=None, square=False):
    ...
    with torch.no_grad():
        ...
        if all([i == j for i, j in zip(net_output.shape, gt.shape)]):
            ...
            y_onehot.scatter_(1, gt, 1)
            onehot_out = y_onehot
            import nibabel as nib  # debug
            pred_masks_cpu = onehot_out.cpu().detach().numpy().astype(np.float32)
            pred_masks_last_3_dims = pred_masks_cpu[0, 0, :, :, :]
            nifti_img = nib.Nifti1Image(pred_masks_last_3_dims, affine=None)
            nib.save(nifti_img, '/home/changbo/Segmentation/3D-TransUNet/onehot_out.nii.gz')
    import nibabel as nib  # debug
    pred_masks_cpu = net_output.cpu().detach().numpy().astype(np.float32)
    pred_masks_last_3_dims = pred_masks_cpu[0, 0, :, :, :]
    nifti_img = nib.Nifti1Image(pred_masks_last_3_dims, affine=None)
    nib.save(nifti_img, '/home/changbo/Segmentation/3D-TransUNet/Lossiou.nii.gz')
    tp = net_output * y_onehot
    import nibabel as nib  # debug
    pred_masks_cpu = tp.cpu().detach().numpy().astype(np.float32)
    pred_masks_last_3_dims = pred_masks_cpu[0, 2, :, :, :]
    nifti_img = nib.Nifti1Image(pred_masks_last_3_dims, affine=None)
    nib.save(nifti_img, '/home/changbo/Segmentation/3D-TransUNet/Losstp.nii.gz')
    fp = net_output * (1 - y_onehot)
    fn = (1 - net_output) * y_onehot
    import nibabel as nib  # debug
    pred_masks_cpu = fn.cpu().detach().numpy().astype(np.float32)
    pred_masks_last_3_dims = pred_masks_cpu[0, 2, :, :, :]
    nifti_img = nib.Nifti1Image(pred_masks_last_3_dims, affine=None)
    nib.save(nifti_img, '/home/changbo/Segmentation/3D-TransUNet/Lossfn.nii.gz')
...
class SoftDiceLoss(nn.Module):
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        if self.apply_nonlin is not None:
            x = self.apply_nonlin(x)
            import nibabel as nib  # debug
            pred_masks_cpu = x.cpu().detach().numpy().astype(np.float32)
            pred_masks_last_3_dims = pred_masks_cpu[0, 3, :, :, :]
            nifti_img = nib.Nifti1Image(pred_masks_last_3_dims, affine=None)
            nib.save(nifti_img, '/home/changbo/Segmentation/3D-TransUNet/SoftDiceLossLossPredInput.nii.gz')
        ...
        # If do_bg is false, it indicates that the background is not processed and only the target object is divided
        if not self.do_bg:
            if self.batch_dice:
                dc = dc[1:]
            else:
...
class MCCLoss(nn.Module):
    def __init__(self, apply_nonlin=None, batch_mcc=False, do_bg=True, smooth=0.0):
        """
        Based on the Matthews correlation coefficient
...
class SoftDiceLossSquared(nn.Module):
    def __init__(self, apply_nonlin=None, batch_dice=False, do_bg=True, smooth=1.):
        """
       Square the term in the denominator, as suggested by Milletari et al.
        """
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        if self.apply_nonlin is not None:
            ...
            nifti_img = nib.Nifti1Image(pred_masks_last_3_dims, affine=None)
            nib.save(nifti_img, '/home/changbo/Segmentation/3D-TransUNet/SoftDiceLossSquaredLossPredInput.nii.gz')
...
class DC_and_CE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum", square_dice=False, weight_ce=1, weight_dice=1,
        ...
        """
        Note that the weight of CE and Dice does not need to sum to 1 and can be set to any value
    ...
    def forward(self, net_output, target):
        ...
        self.device = device
        active_loss = SemiActiveLoss(device=self.device)(target,net_output)
        ###########
        ...
        if self.aggregate == "sum":
            result = self.weight_ce * ce_loss + self.weight_dice * dc_loss + active_loss
        else:
...
class DC_and_BCE_loss(nn.Module):
    def __init__(self, bce_kwargs, soft_dice_kwargs, aggregate="sum"):
        """
        Do not use nonlinear transformations in the network, this loss function only applies to the BRATS region
        :param soft_dice_kwargs:
        :param bce_kwargs:
        :param aggregate:
        """

...
def test(fold):
    path= '/results/inference/Task027_ACDC-Tr'
    label_list=sorted(glob.glob(os.path.join(path,'labelsTs','*nii.gz')))
    # infer_list=sorted(glob.glob(os.path.join(path,'inferTs',fold,'*nii.gz')))原来
    infer_list = sorted(glob.glob(os.path.join(path, 'inferimage', '*nii.gz')))

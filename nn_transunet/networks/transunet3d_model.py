...
from .CP_block  import DANetHead
from .CP_block  import CNetHead
from .CP_block  import PNetHead
from .EGA_block import EGA, Out, Up, make_laplace_pyramid
...
class ConvDropoutNormNonlin(nn.Module):
    """
    Fixed an issue where lrelu was used in ConvDropoutNormNonlin instead of nonlin.
    """
...
class StackedConvLayers(nn.Module):
    # The high-level features of the input feature map are extracted by stacking multiple convolution layers
...
def c2_xavier_fill(module: nn.Module) -> None:
    """
    Initialize module.weight with the "XavierFill" implemented in Caffe2 and initialize module.bias to 0

    Args:
        module (torch.nn.Module): The module to initialize.
    """
    # XavierFill in Caffe2 actually corresponds to kaiming_uniform_ in pytorch
...
class Generic_TransUNet_max_ppbp(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
                 ...
                 #####################################
                 # The following are additions based on Generic UNet
                 ...
                 mw = 0.5,
                 is_max=True, is_masked_attn=True, is_max_ds=True, is_masking=True, is_masking_argmax=False,
                 is_fam=False, fam_k=5, fam_reduct_ratio=8,
                 is_max_hungarian=True, num_queries=4, is_max_cls=True,
        ...
        # If convolution upsampling is not used, the output of the last convolution layer must match the number of features of the jump connection, and if convolution upsampling is used, then the reduction of the feature graph will be done by transposing the convolution layer
        ...
        # If you do not want to use dropout in the localization path, set the dropout probability to 0 here
        if not dropout_in_localization:
            old_dropout_p = self.dropout_op_kwargs['p']
            self.dropout_op_kwargs['p'] = 0.0

        # Constructing localization path
        for u in range(num_pool):
            ...
            # The first convolution layer reduces the number of features to match the number of features in the jump connection
            # The following convolution operations will be performed on this feature number
            # If convolution upsampling is not used, the last convolution operation again reduces the number of features
        ...
        ##############################################
        # This is a change from the original Generic UNet
        if self.is_fam:
        ...
        # The correct way to register all modules
        ...
        ################################################################
        # Transformer configuration，Added on the basis of original Generic UNet
        # Whether to apply maximum pooling in a bottleneck layer
        if self.is_max_bottleneck_transformer:
            ...
            # 3D convolution layer maps the output features of the vit to the final output feature map
            ...
            # vis Indicates whether to visualize the attention diagram
        ...
        # Added on the basis of original Generic UNet
        if self.is_max:
        ...
        #################################
                #Added CP module
        # self.da1 = CNetHead(320, 320)
        # self.da2 = CNetHead(256, 256)
        # self.da3 = PNetHead(128, 128)
        # self.da4 = PNetHead(64, 64)
        # self.da5 = PNetHead(32, 32)
        #################################
                # Added EGA module
        self.ega1 = EGA(256)
        self.ega2 = EGA(128)
        ...
        self.up1  = nn.Sequential(nn.Conv3d(256, 256, kernel_size=3, padding=1))
        # self.up2 = Up(128, 64)
        # self.up3 = Up(64, 32)
        # self.up4 = Up(32, 32)
    ...
    def forward(self, x):
        ...
        #################################
        # Added EGA module
        edge_feature = make_laplace_pyramid(x, 5, 1) # level=5 channel=1
        ...
        for u in range(len(self.tu)):
            if  u<len(self.tu)-1 and isinstance(self.is_fam, str) and self.is_fam.startswith('fam_down'): # 未执行
            ...
            if isinstance(self.is_fam, bool) and self.is_fam:  # 未执行
                x_align = self.fams[u](x, x_l=skips[-(u + 1)])
                x = x + x_align
            #####################################
            #            Added CP module
            # if skips[-(u + 1)].size(1) == 320:
            #     skips[-(u + 1)] = self.da1(skips[-(u + 1)])
            # if skips[-(u + 1)].size(1) == 256:
            #     skips[-(u + 1)] = self.da2(skips[-(u + 1)])
            # if skips[-(u + 1)].size(1) == 128:
            #     skips[-(u + 1)] = self.da3(skips[-(u + 1)])
            # if skips[-(u + 1)].size(1) == 64:
            #     skips[-(u + 1)] = self.da4(skips[-(u + 1)])
            # if skips[-(u + 1)].size(1) == 32:
            #     skips[-(u + 1)] = self.da5(skips[-(u + 1)])
            ####################################
            #################################
            #         Added EGA module
            if skips[-(u + 1)].size(1) == 256:
                d1 = self.up1(skips[-(u + 1)])
                out1 = self.out1(d1)
                skips[-(u + 1)] = self.ega1(edge_feature[0], skips[-(u + 1)], out1)
            if skips[-(u + 1)].size(1) == 128:
                # d2 = self.up2(skips[-u], skips[-u])
                out2 = self.out2(x)
                skips[-(u + 1)] = self.ega2(edge_feature[1], skips[-(u + 1)], out2)
            if skips[-(u + 1)].size(1) == 64:
                # d3 = self.up3(skips[-u], skips[-u])
                out3 = self.out3(x)
                skips[-(u + 1)] = self.ega3(edge_feature[2], skips[-(u + 1)], out3)



            # if skips[-(u + 1)].size(1) == 32:
            #     # d4 = self.up4(skips[-u], skips[-u])
            #     out4 = self.out4(x)
            #     skips[-(u + 1)] = self.ega4(edge_feature[3], skips[-(u + 1)], out4)
            #################################
    ...
    def compute_approx_vram_consumption(patch_size, num_pool_per_axis, base_num_features, max_num_features,
                                        ...
                                        conv_per_stage=2):
        """
       Applicable only if num_conv_per_stage and convolutional_upsampling=True
        Not true vram consumption. Just a constant term, proportional to vram consumption (+ offset of parameter storage)
...
class HungarianMatcher3D(nn.Module):
    """Calculate the assignment between the network's goals and predictions

    For efficiency reasons, no_object is not included in the target. Therefore, in general, the number of predictors is more than the number of targets. In this case, the best prediction is matched one-to-one and no other prediction is matched
    """

...
class WarmupPolyLR(_LRScheduler):
    """In a certain number of iterations, the learning rate is linearly increased so that it gradually increases between the two boundaries"""
    def __init__(
...
def dispatch_clip_grad(parameters, value: float, mode: str = 'norm', norm_type: float = 2.0):
    ...
    if mode == 'norm':
        ...
        torch.nn.utils.adaptive_clip_grad(parameters, value, norm_type=norm_type)
    else:
...
def download_from_hdfs(src_path, dst_path, raise_exception=False):
    ...
    src_path specifies the hdfs path to be downloaded, dst_path specifies the local path to be saved, and raise_exception=False determines whether an exception is raised when an error occurs
    """
    if not has_hdfs_path_prefix(src_path): # Checks if the src path prefix is present, and raises an exception if it is not
        raise ValueError(
            'Input src_path {} is not a valid hdfs path'.format(src_path))
    if has_hdfs_path_prefix(dst_path): # Checks whether the dst path has a prefix and, if so, raises an exception
        raise ValueError(
    ...
    try:
        ...
        check_call_hdfs_command(cmd)
        return True
    except Exception as e: # Any exceptions are caught and stored in variable e

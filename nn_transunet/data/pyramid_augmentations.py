...
class RemoveRandomConnectedComponentFromOneHotEncodingTransform(AbstractTransform):
    # Used to randomly remove connected components from one-hot encoded data
...
class MoveSegAsOneHotToData(AbstractTransform):
    # The segmentation results in the channel are converted to one-hot encoding and added to the data
...
class ApplyRandomBinaryOperatorTransform(AbstractTransform):
    # Random application of binary operators to change data can be used for data enhancement or to change data characteristics
...
class ApplyRandomBinaryOperatorTransform2(AbstractTransform):
    # Randomly apply expansion operations to change the data and remove new categories added in the channel

...
class GenericPreprocessor(object):
    ...
    def resample_and_normalize(self, data, target_spacing, properties, seg=None, force_separate_z=None):
        ...
        for c in range(len(data)):
            ...
            if scheme == "CT":
                # Crop the data to the lower limit (lb) and upper limit (ub) of the training data's foreground and use the training data's foreground mean (mn) and standard deviation (sd)
    ...
    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, data_identifier,
        ...
        # The need to know which categories exist in the data set so that the positions of those categories can be calculated in advance is a must for an oversampled foreground
...
class Preprocessor3DBetterResampling(GenericPreprocessor):
    """
   This preprocessing uses force_separate_z=False. For the data (same as the GenericPreprocessor) it uses a three-level spline for resampling the target spacing,
For split results (unlike GenericPreprocessor). He doesn't do public resampling on the Z-axis
    """

    def resample_and_normalize(self, data, target_spacing, properties, seg=None, force_separate_z=False):
        """
        data and seg must be transposed by transpose_forward. properties are values of 1 that are not transposed (e.g. spacing, etc.)
        :param data:
        :param target_spacing:
        :param properties:
        :param seg:
...
class PreprocessorFor3D_LeaveOriginalZSpacing(GenericPreprocessor):
    """
   3d_lowres and 3d_fullres do not require resampling on the Z-axis
    """

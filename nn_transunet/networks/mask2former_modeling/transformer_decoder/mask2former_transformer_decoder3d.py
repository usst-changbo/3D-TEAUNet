...
from fvcore.common.registry import Registry  # 用于创建和管理注册表，并通过名称进行访问和检索

from .position_encoding import PositionEmbeddingSine
from torch.cuda.amp import autocast  # 上下文管理器，用于自动执行混合精度计算

from nn_transunet.networks.d2util import configurable, Conv2d
from nn_transunet.networks.vit_modeling import LayerScale

TRANSFORMER_DECODER_REGISTRY = Registry("TRANSFORMER_MODULE") # Create a registry to manage the transformer module, which can be obtained using the get method

class SelfAttentionLayer(nn.Module):
    # The attention score between a query vector at each position of the input data and the keys at all positions is calculated, and the weighted summation is performed according to the attention score
    ...
    def __init__(self, d_model, nhead, dropout=0.0,
        ...
        self.ls1 = LayerScale(d_model, init_values=1e-5) if use_layer_scale else nn.Identity() # nn.Identity() means that nothing is done, just that the tensor is returned as-is
...
class CrossAttentionLayer(nn.Module):
    # When processing data, the query interacts with the relevant context to obtain more complete semantic information
...
class FFNLayer(nn.Module):
    # Feedforward neural networks model nonlinear changes and contexts of input features to capture complex relationships between models
...
@TRANSFORMER_DECODER_REGISTRY.register() # A decorator that registers a function or class as a Transformer decoder module
class MultiScaleMaskedTransformerDecoder3d(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.pe_layer = PositionEmbeddingSine(N_steps, normalize=True)  # Generating position coding
        ...
        for _ in range(self.num_layers):
            self.transformer_self_attention_layers.append(
                SelfAttentionLayer(
                    ...
                    is_mhsa_float32=is_mhsa_float32, # new
                    use_layer_scale=use_layer_scale, # new
                )
            )
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, mask_classification=True):
        ...
        # bqc,bcdhw->bqdhw indicates that when the two tensors are multiplied, the sum and shape transformation are performed according to the specified dimension, where b is batch_size, c is not the number of channels, and dhw represents the depth, height and width respectively
        outputs_mask = torch.einsum("bqc,bcdhw->bqdhw", mask_embed, mask_features)
        attn_mask = F.interpolate(outputs_mask, size=attn_mask_target_size, mode="trilinear", align_corners=False)  # 插值

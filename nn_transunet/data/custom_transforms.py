...
class ConvertSegmentationToRegionsTransform(AbstractTransform):
    def __init__(self, regions: dict, seg_key: str = "seg", output_key: str = "seg", seg_channel: int = 0):
        """
        regions Is a tuple within a tuple, and each inner tuple contains a class index that is merged into a region, for example:
        regions= ((1, 2), (2, )) Two regions are generated, one covering the region labeled 1 and 2, and the other covering only 2
        :param regions:
        :param seg_key:
        :param output_key:
        """
